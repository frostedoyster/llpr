...
def test_consistency(bias):
    """Tests the consistency of the various implementations
    ...
    assert torch.allclose(predictions_llpr, predictions_pr)
    assert torch.allclose(predictions_llpr, predictions_pr_loss_agnostic)
    assert torch.allclose(uncertainties_llpr, uncertainties_pr)
    assert torch.allclose(uncertainties_llpr, uncertainties_pr_loss_agnostic)
