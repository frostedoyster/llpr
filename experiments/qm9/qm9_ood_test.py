...
ood_dataloader = torch.utils.data.DataLoader(ood_dataset, batch_size=batch_size, collate_fn=collate_fn)

state_dict = torch.load("outputs/models/qm9_ood.pt")
...
actual_errors = []
with torch.no_grad():
    for batch in tqdm.tqdm(ood_dataloader):
        input, result = batch 
        prediction, uncertainty = model_with_uncertainty(input)
        estimated_errors.append(uncertainty)
        actual_errors.append(
...
plt.plot(estimated_averages, actual_averages, ".", markersize=5.0, color="tab:red")
plt.fill_between([0.0, max_value], [0.0, max_value+n_sigma*np.sqrt(2.0/n_samples_per_bin)*max_value], [0.0, max_value-n_sigma*np.sqrt(2.0/n_samples_per_bin)*max_value], alpha=0.3, color="tab:orange")
plt.xscale("log")
plt.yscale("log")
plt.xlabel("Predicted variance")
